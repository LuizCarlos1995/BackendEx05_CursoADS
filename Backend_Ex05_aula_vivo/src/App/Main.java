...
public class Main {
	...
	private static final String URL = "jdbc:mysql://localhost:3306/jbdc_teste3";
	private static final String USUARIO = "root";
	private static final String SENHA = "@Scorpions2010";
	...
	public static void main(String[] args) {
		...
		try (Connection conexao = DriverManager.getConnection(URL, USUARIO, SENHA)) {
			...
			inserirRegistro(conexao, "Luiz carlos", 29, "Analise e desenvolvimento de sistema");

			// Editando um registro existente (alterar curso para o ID 1)
			editarRegistro(conexao, 5, "Engenharia de Controle e Automação");
	...
	private static void inserirRegistro(Connection conexao, String nome, int idade, String curso) throws SQLException {
		String sql = String.format("INSERT INTO Alunos (nome, idade, curso) VALUES ('%s', %d, '%s')", nome, idade, curso);
	...
	private static void editarRegistro(Connection conexao, int id, String novoCurso) throws SQLException {
		String sql = String.format("UPDATE Alunos SET curso = '%s' WHERE id = %d", novoCurso, id);
		try (Statement stmt = conexao.createStatement()) {
			int linhasAfetadas = stmt.executeUpdate(sql);
